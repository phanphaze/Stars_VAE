...
from src.model import CAE, VAE
from src.utils import save_model
from src.config import Learning_rate, num_epochs, early_stopping_min_delta, early_stopping_patience, lambda_value

def vae_loss_function(reconstructed, original, mu, logvar):
    mse = F.mse_loss(reconstructed, original, reduction="sum")
    
    # KLD calculated for logging, but isolated from the optimization gradient
    kld = -0.5 * torch.sum(1 + logvar - mu.pow(2) - logvar.exp())

    total_loss = lambda_value * mse
    return total_loss, mse, kld
...
def train_model(data, model="VAE"):
    ...
    for epoch in range(num_epochs):
        ...
        total_train_loss = train_mse_loss
        total_val_loss = val_mse_loss

        if prev_val_loss is not None and total_val_loss > prev_val_loss - early_stopping_min_delta:
            patience_counter += 1
        else:
            patience_counter = 0
        ...
        print(
            f"Epoch {epoch} | Train MSE: {train_mse_loss:.4f} | Train KLD (Null): {train_kld_loss:.4f} | Total Train: {total_train_loss:.4f}"
        )
        print(
            f"       {" " * (len(str(epoch)))} | Val MSE: {val_mse_loss:.4f} | Val KLD (Null): {val_kld_loss:.4f} | Total Val: {total_val_loss:.4f}"
        )
